...
function ezCollections:GetCharacterCameraID(invTypeName, cameraID)
    ...
    -- [shell] These tables only know the stock races/sexes/slots. A race added
    -- by a custom DBC (or any token that does not match a key EXACTLY) made
    -- every lookup below arithmetic on nil. That error aborts SetupDressUpModel
    -- in the MIDDLE of DressUpFrame_Show, which shows the window BEFORE it
    -- dresses the model -- leaving the Dressing Room open on an undressed
    -- character. Degrade to the default camera instead: GetUICameraInfo()
    -- already treats a nil id as "no camera" and the caller falls back to the
    -- default view, so a missing camera costs framing, never the outfit.
    option = option or fallback;
    race = race or self.RaceToCameraID.Human;
    sex = sex or self.SexToCameraID[2];
    if not (option and race and sex) then return nil; end
    if cameraID then
    ...
    if not invType then return nil; end   -- [shell] unknown slot name, see above
...
function ezCollections:PlayerHasDifferentCameraOptions()
    local race = self.RaceToCameraID[select(2, UnitRace("player"))];
    local sex = self.SexToCameraID[UnitSex("player")];
    if not race or not sex then return false; end   -- [shell] see GetCharacterCameraID
