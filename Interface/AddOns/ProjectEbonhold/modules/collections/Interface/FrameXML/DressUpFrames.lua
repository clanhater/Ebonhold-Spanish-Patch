...
-- [ebonhold] The 3.3.5 client ships its OWN Dressing Room in patch-D.mpq
-- (Interface\FrameXML\DressUpFrame.lua/.xml), and that file's DressUpItemLink
-- steps aside only for an addon literally named "ezCollections". This addon
-- is ProjectEbonhold, so the handoff never fires and we do NOT take the
-- Dressing Room over: this guard stops the file before it nils the native
-- DressUpFrame/DressUpModel globals, and DressUpFrames.xml is left out of
-- the .toc entirely. Everything below it is the ezCollections replacement
-- window, kept in the tree for reference only.
if ADDON_NAME ~= "ezCollectionsDressUp" then
	...
	-- [ebonhold] GetFrameAndSetBackground() sits ABOVE this guard and calls
	-- SetDressUpBackground, but the real one is defined below it and therefore
	-- never loads -- that was a nil-call on every item preview. A no-op is the
	-- correct body here, not a port of the ezCollections one: with the native
	-- Dressing Room the client owns its own background (DressUpFrame_OnLoad ->
	-- ModelBackground:SetAtlasTex("dressingroom-background-"..class), see
	-- Interface\FrameXML\DressUpFrame.lua in patch-D.mpq), so the addon must
	-- not repaint it.
...
function DressUpModel_OnLoad(self)
	...
	function self:SetCreature(creature, ...)
		self.unit = creature;
		if type(creature) ~= "number" or creature == 0 then return; end -- [shell] no valid creature id
...
local TRANSMOGRIFY_TOOLTIP_APPEARANCE_KNOWN_CHECKMARK = [[|TInterface\AddOns\ProjectEbonhold\modules\collections\interface\Common\CommonIcons:16:16:0:-1:2048:1024:1:257:517:773|t ]]..TRANSMOGRIFY_TOOLTIP_APPEARANCE_KNOWN;
...
function DressUpOutfitDetailsSlotMixin:SetIllusion(transmogID)
	...
	if isHiddenVisual then
		icon = [[Interface\AddOns\ProjectEbonhold\modules\collections\Textures\EnchantSlotIcon]];
	end
