...
StaticPopupDialogs["CONFIRM_REVERT_TRANSMOG"] = {
	text = "¿Revertir toda la transfiguración? Esto elimina todas las apariencias aplicadas (gratis).",
	button1 = YES,
	button2 = NO,
	OnAccept = function (self) C_Transmog.ClearAllTransmog(); end,
	OnCancel = function (self) end,
	hideOnEscape = 1,
	timeout = 0,
	whileDead = 1,
...
-- Apply is the only wardrobe action that costs gold and leaves the client, and a
-- misclick is paid for at 50 to 300 gold a slot, so it asks first. The summary
-- line (slot count + price) is built by WardrobeTransmogFrame_ConfirmApplyPending
-- and passed in as text_arg1.
StaticPopupDialogs["CONFIRM_APPLY_TRANSMOG"] = {
	text = "%s",
	button1 = YES,
	button2 = NO,
	OnAccept = function (self) WardrobeTransmogFrame_ApplyPending(0); end,
	OnCancel = function (self) end,
	-- Apply is disabled while this is up (see WardrobeTransmogFrame_UpdateApplyButton),
	-- so it has to be re-evaluated on the way out, whichever button closed it.
	OnHide = function (self) WardrobeTransmogFrame_UpdateApplyButton(); end,
	hideOnEscape = 1,
	timeout = 0,
	whileDead = 1,
}
