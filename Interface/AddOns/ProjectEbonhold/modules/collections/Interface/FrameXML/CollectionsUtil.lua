		...
		for _, setID in ipairs(info.Sets) do
			...
			if ezCollections.Config.TooltipSets.SlotStateStyle > 1 then
				if slotCollectedFromAnotherSource then
					...
					iconTexture = [[Interface\AddOns\ProjectEbonhold\modules\collections\Textures\TooltipIcons\KNOWN_circle]];
				...
				line:SetText([[|TInterface\AddOns\ProjectEbonhold\modules\collections\Interface\Common\spacer:100:1|t]]);
...
function CollectionWardrobeUtil.SetAppearanceTooltip(tooltip, sources, primarySourceID, selectedIndex, showUseError, inLegionArtifactCategory, subheaderString, showSets, transmogType)
	...
	if ( #sources == 0 ) then
		-- e.g. the "hide" entry, or an appearance with no known sources
		GameTooltip_SetTitle(tooltip, ezCollections:GetHiddenVisualItemName(), NORMAL_FONT_COLOR);
		tooltip:Show();
		return;
	end
