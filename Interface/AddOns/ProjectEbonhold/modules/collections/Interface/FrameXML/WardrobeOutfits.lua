...
function WardrobeOutfitDropDownTemplate_OnShow(self)
	...
	self:SelectOutfit(self:GetLastOutfitID());  -- show active outfit; don't re-apply it on open
end
...
function WardrobeOutfitDropDownTemplate_OnEvent(self, event)
	...
	if ( event == "TRANSMOG_OUTFITS_CHANGED" ) then
		...
		-- The server is authoritative about which outfit is active ("everything
		-- server side"), so prefer its answer over a stale local selection —
		-- otherwise a nil/old selectedOutfitID from before the data arrived
		-- sticks as "No Outfit" even though the server reports an active outfit.
		self:SelectOutfit(self:GetLastOutfitID() or self.selectedOutfitID);
		if ( WardrobeOutfitFrame:IsShown() ) then
			WardrobeOutfitFrame:Update();
		end
	end
	-- Re-detect the active outfit once transmog data has loaded/changed. On login the
	-- initial match can run before the slot data arrives, leaving "No Outfit"; this
	-- retries whenever the transmog updates until an outfit is selected.
	local serverActive = self:GetLastOutfitID();
	if ( serverActive and serverActive ~= self.selectedOutfitID ) then
		self:SelectOutfit(serverActive);
	elseif ( not self.selectedOutfitID ) then
		self:SelectOutfit(serverActive);
	end
...
function WardrobeOutfitDropDownMixin:GetLastOutfitID()
	-- default the dropdown to the outfit matching the currently-applied transmog
	...
	for i = 1, C_TransmogCollection.GetNumMaxOutfits() do
		...
		if ( outfits[i] or newOutfitButton ) then
			...
			if ( newOutfitButton ) then
				...
				button.Icon:SetTexture([[Interface\AddOns\ProjectEbonhold\modules\collections\Interface\PaperDollInfoFrame\Character-Plus]]);
				button.outfitID = nil;
				button.Check:Hide();
