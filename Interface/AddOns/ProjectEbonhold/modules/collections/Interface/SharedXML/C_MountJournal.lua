...
-- [shell] The upstream StaticPopup definitions for these dialogs never made
-- it into the port: ShowMacroPopup indexed StaticPopupDialogs["..."] (nil)
-- and the whole favorite-mount-macro drag flow errored out. Defined here
-- (before any use; C_PetJournal reuses EZCOLLECTIONS_ERROR too).
...
StaticPopupDialogs["EZCOLLECTIONS_MOUNT_MACRO_PERCHARACTER"] = {
    text = "No hay casillas de macros para toda la cuenta libres. ¿Deseas crearla como una macro específica del personaje?",
    button1 = ACCEPT, button2 = CANCEL,
    timeout = 0, whileDead = true, hideOnEscape = true, preferredIndex = 3,
};
...
local function IsMountUsable(mountID)
    -- [shell] NO client-side usability checks on Ebonhold: any mount is
    -- summonable anywhere, anytime (no flyable-area gate, no outdoors gate,
    -- no swimming gate, no AQ special-case). The random picker still ORDERS
    -- by situation -- flying mounts favored only in flyable areas -- and the
    -- server remains the final authority on whether a summon succeeds.
    return true, nil;
end
...
-- [shell] Dead simple, by design: the random button casts a uniformly random
-- FAVORITE mount -- no usability checks, no flying/swimming priority tiers.
-- Falls back to any collected mount when no favorite is set.
local function SelectRandomFavoriteMount()
    local favorites, all = {}, {};
    local favContainer = ezCollections:GetMountFavoritesContainer();
    for companionIndex = 1, GetNumCompanions("MOUNT") do
        ...
        if favContainer[mountID] then
            table.insert(favorites, mountID);
        end
    end
    local pool = (#favorites > 0) and favorites or all;
    if #pool > 0 then
