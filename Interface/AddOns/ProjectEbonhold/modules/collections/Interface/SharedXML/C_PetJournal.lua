...
-- [shell] Same gap as the mount side: the upstream StaticPopup definitions for
-- these dialogs never made it into the port, so ShowMacroPopup indexed
-- StaticPopupDialogs["..."] (nil) and the favorite-pet-macro drag flow errored
-- out. EZCOLLECTIONS_ERROR is defined in C_MountJournal.lua, which the TOC
-- loads first.
StaticPopupDialogs["EZCOLLECTIONS_PET_MACRO_CREATE"] = {
    text = "Arrastrar este botón crea una macro que invoca una mascota favorita aleatoria. ¿Deseas crearla?",
...
function C_PetJournal.RefreshPets() -- Custom
    ...
    for petID, info in pairs(ezCollections.Cache.Pets) do
        if type(petID) == "number" then
            local _, _, _, name = unpack(info);
            if name and not tContains(_petIDs, petID) then
                table.insert(_petIDs, petID);
            end
        end
    end
