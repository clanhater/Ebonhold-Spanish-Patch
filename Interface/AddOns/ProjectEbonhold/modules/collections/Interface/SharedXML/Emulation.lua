...
function GameTooltip:SetToyByItemID(itemID)
	if not itemID then return self:IsShown(); end -- [shell] empty slot: no toy, no tooltip
	...
	if self:IsShown() then
		...
		if ezCollections.Developer then
			self:AddLine("ID del juguete: "..toyID, 0.5, 0.5, 0.5);
			self:AddLine("ID del objeto: "..itemID, 0.5, 0.5, 0.5);
		end
