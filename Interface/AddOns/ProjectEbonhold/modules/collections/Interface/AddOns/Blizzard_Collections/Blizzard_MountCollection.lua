...
function MountJournal_OnShow(self)
	...
	MountJournal_UpdateMountDisplay();
	SetPortraitToTexture(CollectionsJournalPortrait, [[Interface\AddOns\ProjectEbonhold\modules\collections\Interface\Icons\MountJournalPortrait]]);
...
function MountJournal_UpdateMountList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedMounts and showMounts ) then
			...
			button.name:SetText("");
			button.icon:SetTexture([[Interface\AddOns\ProjectEbonhold\modules\collections\Interface\PetBattles\MountJournalEmptyIcon]]);
			button.new:Hide();
			button.newGlow:Hide();
	...
	if ( MountJournal.selectedMountID ) then
		...
		if creatureName and ( MountJournal.MountDisplay.lastDisplayed ~= spellID ) then
			...
			if creatureDisplayID == 0 then
				...
				-- [shell] SetCreature() draws nothing when the creature is not
				-- in the local cache (creaturecache.wdb), which the client
				-- cannot fill on its own. Ask the server for it and redraw once
				-- the answer lands; both calls no-op after the first time (the
				-- request is deduped per session, the cache is persisted).
				if ProjectEbonhold and ProjectEbonhold.RequestCreatureCache then
					local model = MountJournal.MountDisplay.ModelFrame;
					local loaded = model:GetModel();
					if not loaded or loaded == "" or loaded == "none" then
						if ProjectEbonhold.RequestCreatureCache(creatureDisplayID) then
							local shown = spellID;
							C_Timer.After(1.0, function()
								if MountJournal.MountDisplay.lastDisplayed == shown and model:IsShown() then
