...
function WardrobeFrame_OnLoad(self)
	...
	SetPortraitToTexture(WardrobeFramePortrait, [[Interface\AddOns\ProjectEbonhold\modules\collections\Interface\Icons\INV_Arcane_Orb]]);
	WardrobeFrameTitleText:SetText(TRANSMOGRIFY);
	PanelTemplates_SetNumTabs(self, 3); -- [shell] Toy/Library/Appearances tabs removed
	PanelTemplates_SetTab(self, 3);     -- Transmogrify keeps id 6 (now named Tab3)
end
...
function WardrobeTransmogFrame_OnLoad(self)
	...
	-- [shell] "Apply to specialization" removed: no spec-scoped transmog on
	-- this server -- Apply always applies outright. Show() neutralized so no
	-- stock path can bring the button (or its tutorial bubble) back.
	if self.SpecButton then
		self.SpecButton:Hide();
		self.SpecButton.Show = function() end;
	end
	if self.SpecHelpBox then
		self.SpecHelpBox:Hide();
		self.SpecHelpBox.Show = function() end;
	end
...
function WardrobeTransmogFrame_OnEvent(self, event, ...)
	if ( event == "TRANSMOGRIFY_UPDATE" or event == "TRANSMOGRIFY_ITEM_UPDATE" ) then
		...
		if ( slotButton ) then
			...
			if ( hasUndo ) then
				PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_VoidStorage_Undo.wav]]);
			elseif ( not hasPending ) then
				if ( slotButton.hadUndo ) then
					PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_VoidStorage_Redo.wav]]);
		...
		if ( event == "TRANSMOGRIFY_UPDATE" ) then
			...
			-- Its "N slots for X gold" line was computed from the pending set
			-- that just changed underneath it.
			StaticPopup_Hide("CONFIRM_APPLY_TRANSMOG");
	...
	elseif ( event == "UNIT_MODEL_CHANGED" ) then
		...
		if ( unit == "player" and WardrobeTransmogFrame.Model:CanSetUnit("player") ) then
			...
			if ( self.inAlternateForm ~= inAlternateForm ) then
				...
				WardrobeTransmogFrame.Model:SetAutoDress(false);
...
function WardrobeTransmogFrame_OnShow(self)
	ezCollections:SetCVar("petJournalTab", 3);
...
function WardrobeTransmogFrame_OnHide(self)
	if ezCollections.Config.Wardrobe.EtherealWindowSound then
		PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_EtherealWindow_Close.wav]]);
	else
		PlaySound("igCharacterInfoClose");
	end
	StaticPopup_Hide("TRANSMOG_APPLY_WARNING");
	StaticPopup_Hide("CONFIRM_APPLY_TRANSMOG");  -- nothing left to confirm against
...
function WardrobeTransmogFrame_UpdateSlotButton(slotButton)
	...
	if ( hasChange and (hasUndo or isPendingCollected and not slotButton.invalidWeapon) and not slotButton.AnimFrame:IsShown() ) then
		...
		if slotFailReason then
			...
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\ProjectEbonhold\modules\collections\Textures\RedIconAlertAnts]]);
			slotButton.failed = true;
		else
			slotButton.PendingFrame.Glow:SetVertexColor(1, 1, 1);
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\ProjectEbonhold\modules\collections\Interface\Transmogrify\PurpleIconAlertAnts]]);
		end
		if ( hasUndo ) then
	...
	if ( showModel ) then
		...
		if ( sourceID == NO_TRANSMOG_SOURCE_ID or C_TransmogCollection.IsAppearanceHiddenVisual(sourceID) ) then
			-- hide: leave the slot undressed (the full Undress() ran before this loop)
...
function WardrobeTransmogFrame_UpdateApplyButton()
	...
	if not cost and not tokens then
		...
		WardrobeTransmogFrame.Model.ClearAllPendingButton:SetShown(true);  -- always-available Revert button
		return;
	end
	...
	if ( StaticPopup_FindVisible("TRANSMOG_APPLY_WARNING") or StaticPopup_FindVisible("CONFIRM_APPLY_TRANSMOG") ) then
		canApply = false;
	end
	-- Un-owned previews no longer block Apply; GetCost only counts applyable
	-- (owned + hide) changes, so Apply stays enabled to apply the rest.
...
-- The Apply button's confirmation gate: it prices the pending set and asks
-- before anything is charged. Only the button routes through here, on purpose.
-- The apply warnings chain, the retry after item info arrives, and the
-- confirmation's own OnAccept all reach WardrobeTransmogFrame_ApplyPending
-- directly, so a confirmed apply is never asked about twice.
function WardrobeTransmogFrame_ConfirmApplyPending()
	local cost, numChanges = C_Transmog.GetCost();
	cost = cost or 0;
	numChanges = numChanges or 0;
	-- Belt and braces: the button is disabled at 0 changes, but a stale click
	-- must not put up a popup that would apply nothing.
	if ( numChanges == 0 ) then
		WardrobeTransmogFrame_UpdateApplyButton();
		return;
	end
...
function WardrobeTransmogFrame_OnTransmogApplied()
	PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_Transmogrify_Apply.wav]]);
...
function WardrobeOutfitMixin:GetLastOutfitID()
	-- The server tracks which outfit is active ("everything server side"); use it
	-- directly. (The old body returned a client CVar and, when OutfitsSelectLastUsed
	-- was off, `return;` yielded NO value -> SelectOutfit got nil -> stuck "No Outfit".)
	return C_TransmogCollection.GetActiveOutfitID and C_TransmogCollection.GetActiveOutfitID() or nil;
end
...
function WardrobeTransmogButton_OnClick(self, button)
	...
	if ( button == "RightButton" ) then
		if ( hasPending or hasUndo ) then
			PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_Reforging_Restore.wav]]);
			C_Transmog.ClearPending(slotID, self.transmogType);
		elseif ( isTransmogrified ) then
			-- right-click a transmogged slot: stage a removal (pending), applied on Apply
			PlaySoundFile([[Interface\AddOns\ProjectEbonhold\modules\collections\Sounds\UI_Reforging_Restore.wav]]);
...
function WardrobeCollectionFrame_OnLoad(self)
	...
	PanelTemplates_SetNumTabs(self, 1); -- [shell] Sets sub-tab removed
...
function WardrobeItemsCollectionMixin:CheckLatestAppearance(changeTab)
	...
	if ( self.latestAppearanceID ~= latestAppearanceID ) then
		...
		if ( changeTab and not CollectionsJournal:IsShown() ) then
			CollectionsJournal_SetTab(CollectionsJournal, 3); -- Transmogrify's id in this shell's renumbering
		end
	end
end
...
function WardrobeItemsCollectionMixin:OnShow()
	...
	if self:GetParent().SetsTabHelpBox then -- [shell] Sets sub-tab removed
...
function WardrobeCollectionFrame_UpdateTabButtons()
	if not WardrobeCollectionFrame.SetsTab then return; end -- [shell] Sets sub-tab removed
	-- sets tab
...
function WardrobeItemsCollectionMixin:FilterVisuals()
	...
	local showCollected = ezCollections:GetCVarBool("transmogrifyShowCollected");
	local showUncollected = ezCollections:GetCVarBool("transmogrifyShowUncollected");
	local isAtTransmogrifier = WardrobeFrame_IsAtTransmogrifier();
	local visualsList = self.visualsList;
	local filteredVisualsList = { };
	-- [shell] cap background item-cache warming to roughly two pages' worth:
	-- warming the WHOLE category fired hundreds of server item queries at
	-- once in large categories. Cells beyond the cap are queried when their
	-- page actually renders (UpdateItems queries visible cells itself).
	local warmBudget = (self.PAGE_SIZE or 18) * 2;
	local function warmCache(visualID)
		if GetItemInfo(visualID) then return; end
		missing = true;
		if warmBudget <= 0 then return; end
		warmBudget = warmBudget - 1;
		ezCollections:QueryItem(visualID);
		if not WardrobeCollectionFrame.visualListCacheRefreshTimer then
			WardrobeCollectionFrame.visualListCacheRefreshTimer = ezCollections.AceAddon:ScheduleRepeatingTimer(function()
				...
				if WardrobeCollectionFrame:IsVisible() then
					stillMissing = not self:RefreshVisualsList();
					self:UpdateItems();
				end
	...
	for i = 1, #visualsList do
		-- [shell] search box: name filter first (the "hide" entry is exempt);
		-- an uncached name warms the item cache so the 0.1s refresh timer
		-- re-filters once it arrives instead of losing the item silently.
		local searchOk, searchNameMissing = true, false;
		if ( C_TransmogCollection.IsSearchMatch and not visualsList[i].isHideVisual ) then
			searchOk, searchNameMissing = C_TransmogCollection.IsSearchMatch(visualsList[i].visualID, self.searchType, visualsList[i].name);
		end
		if ( not searchOk ) then
			if ( searchNameMissing ) then warmCache(visualsList[i].visualID); end
		elseif ( C_TransmogCollection.IsVisualArmorTypeShown and not C_TransmogCollection.IsVisualArmorTypeShown(visualsList[i].visualID) ) then
			-- filtered out by the Armor Type filter
		elseif ( isAtTransmogrifier ) then
			local v = visualsList[i];
			if ( v.isHideVisual ) then
				-- the "no visual / hide slot" entry is always available
				tinsert(filteredVisualsList, v);
			elseif ( v.isCollected ) then
				-- [shell] collected: respect the Collected toggle
				if showCollected then tinsert(filteredVisualsList, v); warmCache(v.visualID); end
			elseif ( v.isUsable and showUncollected ) then
				-- [shell] uncollected but usable: show for preview when the Not-Collected toggle is on
				tinsert(filteredVisualsList, v);
				warmCache(v.visualID);
			elseif not v.isCollected and showClaimable and v.isClaimable then
				tinsert(filteredVisualsList, v);
			elseif not v.isCollected and showPurchasable and (v.isStoreSource or v.isSubscriptionSource) then
				tinsert(filteredVisualsList, v);
			elseif not v.isCollected and showBattlePass and v.isBattlePassSource then
				tinsert(filteredVisualsList, v);
			end
		else
			local hide;
...
function WardrobeCollectionFrame_CanEnchantSource(sourceID)
	...
	-- [shell] trust the category's canEnchant directly: the stock
	-- HiddenModel:HasAttachmentPoints() probe is unreliable on this client
	-- and rejected every weapon -- the illusion grid then fell back to an
	-- uncached placeholder weapon and rendered bare holder bodies.
	return canEnchant and true or false;
end
...
function WardrobeItemsCollectionMixin:UpdateItems()
	...
	for i = 1, self.PAGE_SIZE do
		...
		if ( visualInfo ) then
			...
			if visualInfo.isHideVisual then hasItemData = true; end  -- hide entry isn't a real item
			...
			if self.transmogType == LE_TRANSMOG_TYPE_ILLUSION then
				...
				-- [shell] hand-tuned weapon-rig framing like the appearance
				-- weapon cells (retail's scale 1 rendered the tiny holder
				-- creature at the bottom of the cell). Scale 4, not 6: a page
				-- of 18 enchant GLOWS is particle-heavy, and emitter size
				-- scales with the model -- 4 keeps the weapon readable while
				-- costing visibly less than the appearance grid's 6.
				...
				-- Weapon rig framing: this trigger creature's natural size is much
				-- smaller than a player body, so the DBC weapon camera (tuned for a
				-- player-sized rig at scale 1) framed it too small/off-center here.
				-- Hand-tuned instead (scale 6, fixed position/facing) via /wrigtune.
				model:SetModelScale(isArmor and 5 or 6);
			...
			if ( isArmor ) then
				...
				-- Weapon holder rig: DBC camera math (Model_ApplyUICamera) scales
				-- position by model scale, which also amplifies the tuned camera's
				-- side/height offsets and drags the model sideways as scale grows
				-- (confirmed empirically). Fixed hand-tuned position instead.
				model.cameraID = nil;
				model:SetPosition(1.0, 0, -1.0);
				model:SetFacing(1);
			end
			...
			if needsDressing then
				model:Undress();
				if ( self.transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
					-- [shell] illusions: the cell's own carrier id IS the
					-- source (the appearance resolution below only knows the
					-- appearance catalog and returned 0 -- cells then dressed
					-- the bare weapon with no enchant)
					model.previewedSource = visualInfo.sourceID;
				else
				...
				elseif ( appearanceVisualID ) then
					...
					-- weapon: creature holder + TryOn the appearance (original ezCollections way)
			...
			if ( not visualInfo.isCollected ) then
				...
				-- [shell] ownership readability: the border swap alone is too
				-- subtle to scan -- uncollected cells also get a dark shade
				-- over the 3D preview (texture children of a Model frame
				-- always draw above its 3D render; ARTWORK keeps it under the
				-- OVERLAY border/favorite/new art).
				if ( not model.UncollectedShade ) then
					local shade = model:CreateTexture(nil, "ARTWORK", nil, 0);
					shade:SetAllPoints(model);
					shade:SetTexture("Interface\\Buttons\\WHITE8X8");
					shade:SetVertexColor(0, 0, 0, 0.5);
					model.UncollectedShade = shade;
				end
				model.UncollectedShade:Show();
			elseif ( not visualInfo.isUsable ) then
				model.Border:SetAtlas("transmog-wardrobe-border-unusable");
				if ( model.UncollectedShade ) then model.UncollectedShade:Hide(); end
			else
...
function WardrobeCollectionFrame_UpdateProgressBar(value, max)
	value = value or 0; max = max or 0; -- [shell] empty data: no counts yet
...
function WardrobeItemsCollectionMixin:GetAnAppearanceSourceFromVisual(visualID, mustBeUsable)
	if ( C_TransmogCollection.IsAppearanceHiddenVisual(visualID) ) then
		return ezCollections:GetHiddenVisualItem();
	end
...
function WardrobeItemsCollectionMixin:SelectVisual(visualID)
	...
	local sourceID;
	if ( self.transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
		-- [shell] illusions: source == visual == carrier id (the appearance
		-- source-resolution below only knows the appearance catalog and
		-- returned 0 here, which staged a REMOVAL instead of the illusion)
		sourceID = visualID;
	elseif ( C_TransmogCollection.IsAppearanceHiddenVisual(visualID) ) then
		sourceID = ezCollections:GetHiddenVisualItem();   -- "hide this slot"
	else
...
function WardrobeCollectionFrame_OpenTransmogLink(link, transmogType)
	if ( not CollectionsJournal:IsVisible() or not WardrobeCollectionFrame:IsVisible() ) then
		ToggleCollectionsJournal(3); -- Transmogrify's id in this shell's renumbering
	end
...
function WardrobeItemsCollectionMixin:GoToSourceID(sourceID, slot, transmogType, forceGo)
	...
	if ( visualID or forceGo or slot ) then
		-- slot present but no visualID happens for a hidden slot (the applied "source"
		-- is the hide marker, which has no appearance info); still navigate to the
		-- slot's category so the slot stays clickable.
...
function WardrobeItemsCollectionMixin:RefreshAppearanceTooltip(cycle)
	...
	local sources;
	if ( self.transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
		-- [shell] illusions have no appearance sources -- synthesize one from
		-- the carrier so the tooltip header resolves via GetIllusionSourceInfo
		-- (the empty appearance lookup made every cell read as "Hide").
...
function WardrobeItemsModelMixin:OnUpdate()
	if ( self.type == "main" or self.type == "off" ) then
		-- The weapon-holder creature (a special-purpose trigger rig, not a full
		-- player skeleton) doesn't have the ANIMATION_WEAPON_PREVIEW_MAIN/OFF
		-- sequences (51/63) that hold a real player model in a weapon pose.
		-- SetSequenceTime silently no-ops on an invalid sequence, leaving the
		-- model on its own default (looping) animation instead of held still.
		-- Sequence 0 ("Stand") exists on virtually every creature.
		self:SetSequenceTime(self.cameraAnimID or 0, 0);
	else
...
function WardrobeCollectionFrameWeaponDropDown_Init(self)
	...
	for categoryID = FIRST_TRANSMOG_COLLECTION_WEAPON_TYPE, LAST_TRANSMOG_COLLECTION_WEAPON_TYPE do
		...
		if ( name and isWeapon ) then
			if ( (slot == "MAINHANDSLOT" and canMainHand) or (slot == "SECONDARYHANDSLOT" and canOffHand) ) or slot == "RANGEDSLOT" and canRanged then
				-- [shell] the server permits cross-type weapon transmog, so allow any
				-- weapon category that fits the slot (no equipped-item type restriction).
				if ( true ) then
					info.text = name;
...
function WardrobeFilterDropDown_InitializeItems(self, level)
	...
	if level == 1 then
		...
		-- Classes / Races / Sources / Expansion filters are not backed by the shell's
		-- data (no per-item class/race/source/expansion info), so they're omitted.
		...
		-- [shell] "Purchasable" and "Battle Pass" filters removed (store
		-- concepts this server doesn't use). "Claimable" stays, but only
		-- when there is actually something claimable -- and the separator
		-- only renders when that block does, so it can't dangle alone.
		if atTransmogrifier and next(ezCollections.UnclaimedQuests) then
			UIDropDownMenu_AddSeparator(level);
		...
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 1 then
			...
			for i = 1, numSources do
				-- [shell] "Boss Drop" (and its instance/boss drilldown) is
				-- removed from the source list: not a meaningful item source
				-- on this server's loot model.
						...
						for encounterID in pairs(ezCollections.Encounters) do
							...
							if encounter and encounter.Map == id then
								...
								if instance.Type == INSTANCE_TYPE_DUNGEON then
									prefix = [[|TInterface\AddOns\ProjectEbonhold\modules\collections\Interface\Minimap\Dungeon:22:22:0:0:32:32:5:27:5:27|t]];
								elseif instance.Type == INSTANCE_TYPE_RAID then
									prefix = [[|TInterface\AddOns\ProjectEbonhold\modules\collections\Interface\Minimap\Raid:22:22:0:0:32:32:5:27:5:27|t]];
								end
								if prefix then
...
function WardrobeTransmogFrameSpecDropDown_Initialize()
	...
	info.checked = false;
	info.text = format([[|TInterface\AddOns\ProjectEbonhold\modules\collections\Interface\Common\spacer:1:20:-1:-1|t |T%s:22:22:0:-1|t %s (%s)]], icon, name, GetSpecialization() == 1 and TALENT_SPEC_PRIMARY or TALENT_SPEC_SECONDARY);
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
end
...
function WardrobeSetsCollectionMixin:OnShow()
	...
	if (self:GetParent().SetsTabHelpBox and self:GetParent().SetsTabHelpBox:IsShown()) then
		self:GetParent().SetsTabHelpBox:Hide()
