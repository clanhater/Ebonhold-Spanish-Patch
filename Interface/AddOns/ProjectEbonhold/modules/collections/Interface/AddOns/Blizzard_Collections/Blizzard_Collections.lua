...
function CollectionsJournal_UpdateSelectedTab(self)
	...
	if ToyBox then ToyBox:SetShown(selected == 3); end   -- [shell] Toy Box tab removed
	if Library then Library:SetShown(selected == 4); end -- [shell] Library tab removed
...
function CollectionsJournal_OnShow(self)
	...
	WardrobeFramePortraitButton:UpdateVisibility();
	HideUIPanel(WardrobeFrame);
	-- [shell] guard optional micro-button/minimap tutorial chrome that this
	-- data-decoupled build does not instantiate.
	if CollectionsMicroButtonAlert then CollectionsMicroButtonAlert:Hide(); end
	if ezCollectionsMinimapHelpBox then ezCollectionsMinimapHelpBox:Hide(); end
